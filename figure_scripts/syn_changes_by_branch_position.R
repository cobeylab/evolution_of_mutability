# Produces figure for the manuscript showing the distribution of the fraction of negative changes in mutability
...
metric_column <- factor(metric_column, levels = c('S5F', 'log_S5F'))
...
summary_dataframe <- data.frame(lineage,region=region_column, metric = metric_column, 
...
region_names <- c(
  ...
  # Base Plot
  pl <- ggplot(data = subset(summary_dataframe, region != 'WS' & metric == 'log_S5F'), 
               aes(x=lineage,y=mean_fraction_negative_internal)) + 
    scale_y_continuous(expand = c(0,0), limits = c(-0.05,1.05)) + 
    theme_bw() +
    #theme_classic() + 
    ylab('Frequency of synonymous mutability losses') + 
    xlab('Lineage') +
    ggplot_theme +
...
# ======= Plot with separate results for terminal and internal branches
    ...
    pl_terminal_vs_internal <- pl + 
      geom_linerange(aes(ymin=HPD_llim_terminal, ymax = HPD_ulim_terminal), 
                     colour = 'blue') +
      
      geom_point(aes(y=mean_fraction_negative_terminal),
                 shape = 21, fill = 'royalblue1', alpha = 0.7,size = 2.5)  +

      geom_linerange(aes(ymin=HPD_llim_internal, ymax = HPD_ulim_internal), 
                     colour = 'grey40', position = position_nudge(x=-0.3,y=0)) +
      
      geom_point(aes(y=mean_fraction_negative_internal),
...
####
pdf('syn_logS5F_changes_terminal_vs_internal.pdf',width = 5.9, height = 4.5)
...
pl_trunk_vs_nontrunk <- pl + 
      geom_linerange(aes(ymin=HPD_llim_trunk, ymax = HPD_ulim_trunk), 
                     colour = 'darkorange2', position = position_nudge(x=0.2,y=0)) +
      
      geom_point(aes(y=mean_fraction_negative_trunk),
      ...
      geom_linerange(aes(ymin=HPD_llim_nontrunk, ymax = HPD_ulim_nontrunk), 
                     colour = 'grey40', position = position_nudge(x=-0.2,y=0)) +
      
      geom_point(aes(y=mean_fraction_negative_nontrunk),
                 shape = 21, fill = 'grey25', alpha = 0.7,size = 2.5,
  ...
  ####
  pdf('syn_logS5F_changes_trunk_vs_nontrunk.pdf', width = 5.9, height = 4.5)
