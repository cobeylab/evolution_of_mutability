...
def main(argv):
    ...
    original_fasta_filename = "Doria-Rose_2014_VRC26L.fasta"

    # Original fasta file with dates in sequence names:
    VDJ_VRC26L = pyfasta.Fasta(data_directory + original_fasta_filename)
    VRC26L_ids = VDJ_VRC26L.keys()

    # Open original and alingment file
    original_alignment_filename = "../../results/alignments/VRC26L_MACSE_NT_EDITED.nex"
    original_alignment_file = open(original_alignment_filename, 'r')

    # Opening modified, dated combined nexus file:
    output_filename = "../../results/alignments/VRC26L_final_alignment.nex"
    ...
    for line in original_alignment_file:
        ...
        if seq_name is not None:
            ...
            for i in range(len(VRC26L_ids)):
                # Find the corresponding sequence:
                if VRC26L_ids[i].find(seq_name) is not -1:
                    # Find the sampling date
                    time_point = re.search(r'isolate [0-9]*-', VRC26L_ids[i])
